...
impl<'ra, 'tcx> ResolverExpand for Resolver<'ra, 'tcx> {
    ...
    fn set_span_to_node_id(
        &mut self,
        span_to_node_id: rustc_data_structures::fx::FxHashMap<Span, NodeId>,
    ) {
        self.span_to_node_id = span_to_node_id;
    }
    ...
    fn resolve_macro_invocation(
        ...
        //
        // Stores the span + expansion ID of the annotatable that this inner attribute applies to.
        //
        // We need to find the DefID of this annotatable at a later point in the compilation,
        // and at this stage there is no DefID yet.
...
impl<'ra, 'tcx> Resolver<'ra, 'tcx> {
    ...
    /// # Resolution of inner attributes
    ///
    /// Inner attributes are resolved from inside the module that they are applied to.
    /// This resolves to `a::x`:
    ///
    /// ```ignore (illustrative)
    /// macro x() {}
    ///
    ...
    ///
    /// - We compare the old resolution (`x`) (obtained during initial stage) with the new resolution (`target::x`) (obtained during finalization),
    ///   if the 2 resolutions differ (which they do), we error.
    fn resolve_macro_or_delegation_path<'r>(
        ...
        let parent_scope = if inner_attr.is_some() {
            // Inner attributes are resolved from inside the module. Because there are no insides
            // at this stage yet, we create an empty module, and resolve inside of it.
            &ParentScope::module(
                self.arenas.new_empty_module(Some(parent_scope.module), false),
                self.arenas,
            )
        } else {
            parent_scope
    ...
    pub(crate) fn finalize_macro_resolutions(&mut self, krate: &Crate) {
        ...
        dbg!(&self.block_map);
        dbg!(&self.node_id_to_def_id);

        dbg!(&self.span_to_node_id);

        let get_inner_module = |this: &Self, expn_id, span: Span| {
            let parent_did = this.invocation_parent(expn_id);

            let Some(parent_module) = this.get_module(parent_did.into()) else {
                return ParentScope::module(this.arenas.new_root_module(), this.arenas);
            };
            ...
            let module = child_module.unwrap_or_else(|| this.arenas.new_root_module());

            ParentScope::module(module, this.arenas)
        };
        ...
        for (inner_attr, mut path, path_span, kind, mut parent_scope, initial_res, ns) in
            macro_resolutions
        {
            if let Some((expn_id, span)) = inner_attr {
                parent_scope = get_inner_module(&self, expn_id, span);
            }
